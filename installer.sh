#!/bin/bash
## setup command=wget -q --no-check-certificate https://raw.githubusercontent.com/OwnerPlugins/BootlogoChanger/main/installer.sh -O - | /bin/sh

version='1.0.6'
changelog='Porting to python3'
TMPPATH=/tmp/BootlogoChanger-install
FILEPATH=/tmp/BootlogoChanger-main.tar.gz

echo "Starting BootlogoChanger installation..."

# Determine plugin path based on architecture
if [ ! -d /usr/lib64 ]; then
    PLUGINPATH=/usr/lib/enigma2/python/Plugins/Extensions/BootlogoChanger
else
    PLUGINPATH=/usr/lib64/enigma2/python/Plugins/Extensions/BootlogoChanger
fi
...
if [ $? -ne 0 ]; then
    echo "Failed to download BootlogoChanger package!"
    cleanup
    exit 1
fi
...
if [ $? -ne 0 ]; then
    echo "Failed to extract BootlogoChanger package!"
    cleanup
    exit 1
fi
...
elif [ -d "$TMPPATH/BootlogoChanger-main/usr/lib64/enigma2/python/Plugins/Extensions/BootlogoChanger" ]; then
    cp -r "$TMPPATH/BootlogoChanger-main/usr/lib64/enigma2/python/Plugins/Extensions/BootlogoChanger"/* "$PLUGINPATH/" 2>/dev/null
    echo "Copied from lib64 plugin directory"
elif [ -d "$TMPPATH/BootlogoChanger-main/usr" ]; then
    # Copy entire usr tree
    cp -r "$TMPPATH/BootlogoChanger-main/usr"/* /usr/ 2>/dev/null
