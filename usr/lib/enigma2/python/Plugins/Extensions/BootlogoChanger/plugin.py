                ...
                for bootlogo in xml_bootlogos.firstChild.childNodes:
                    if bootlogo.nodeType == bootlogo.ELEMENT_NODE and bootlogo.localName == "bootlogo":
                        ...
                        if status == "checked":
                            ...
                            if debug:
                                print(
                                    _("[BootlogoChanger] randomization: bootlogo found") + ": " + name)

                items_count = len(item_list)
                if debug:
                    print(
                        "[BootlogoChanger] randomization: items_count: " +
                        str(items_count))
                if items_count != 0:
                    copyfiles = True
                    ...
                    if debug:
                        print(
                            "[BootlogoChanger] randomization: new_logo_index: " +
                            str(new_logo_index))
                    selected_bootlogo = bootlogo_directory + \
                        item_list[new_logo_index] + "/"
                    if debug:
                        print(
                            "[BootlogoChanger] randomization: selected_bootlogo: " +
                            selected_bootlogo)

                    filelist = [
                        x for x in listdir(selected_bootlogo) if x.endswith(".mvi")]
                    if not filelist == []:
                        if delete_old_files:
                            oldfilelist = [
                        ...
                        for file in filelist:
                            if (copyfile(selected_bootlogo + file,
                                         bootlogo_directory)) != 0:
                                copyfiles = False
                        ...
                        if copyfiles:
                            ...
                            print(
                                _("[BootlogoChanger] activating new bootlogo failed") +
                                ": " +
                                item_list[new_logo_index] +
                                "\n")
                    else:
                        print(
                            _("[BootlogoChanger] no bootlogos for randomization selected") + "!\n")
        else:
